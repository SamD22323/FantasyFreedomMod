...
import me.fantasyfreedom.fantasyfreedommod.util.FSync;
import static me.fantasyfreedom.fantasyfreedommod.util.FUtil.playerMsg;
...
public class ChatManager extends FreedomService
{
    ...
    public ChatManager(FantasyFreedomMod plugin)
    {
        super(plugin);
    }
    ...
    {
        String name = sender.getName() + " " + plugin.rm.getDisplay(sender).getColoredTag() + ChatColor.AQUA;
        FLog.info("[FANTASYADMIN] " + name + ": " + message);

        for (Player player : server.getOnlinePlayers())
        {
            if (plugin.al.isAdmin(player))
            {
                player.sendMessage("[" + ChatColor.DARK_RED + "FANTASYADMIN" + ChatColor.GRAY + "] " + ChatColor.GOLD + name + ": " + ChatColor.WHITE + message);
            }
        }
    }
